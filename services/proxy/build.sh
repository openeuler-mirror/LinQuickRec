...
if [ $? -eq 0 ]; then
    ...
    print_info "  ./bin/proxy_server --server_port=8080"
    echo ""
    print_info "To run the client:"
    print_info "  cd ${BUILD_DIR}"
    print_info "  ./bin/proxy_test_client --server=127.0.0.1:8080"
    echo ""
    print_info "To run integration test:"
    print_info "  cd ${BUILD_DIR}"
    print_info "  ./bin/proxy_integration_test"
    echo ""
else
